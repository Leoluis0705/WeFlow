...
class ExportService {
  ...
  /**
   * 从消息内容提取图片 DAT 文件名（备用方法）
   * 注意：优先使用 parseImageDatNameFromRow 从 packed_info_data 提取
   */
  ...
  /**
   * 从数据库行记录的 packed_info_data 字段提取图片 DAT 文件名
   * 这是最可靠的方法，优先使用
   */
  private parseImageDatNameFromRow(row: Record<string, any>): string | undefined {
    ...
    const packed = row.packed_info_data || 
                   ...
                   row.PackedInfoData || 
                   row.PackedInfo || 
                   row.WCDB_CT_packed_info_data || 
                   row.WCDB_CT_packed_info || 
                   row.WCDB_CT_PackedInfoData || 
                   row.WCDB_CT_PackedInfo
      ...
      while (hasMore) {
        ...
        for (const row of batch.rows) {
          ...
          if (localType === 3 && content) {
            ...
            // 优先从 packed_info_data 提取 datName（最可靠）
            imageDatName = this.parseImageDatNameFromRow(row)
            // 如果 packed_info_data 提取失败，尝试从 content XML 提取（备用）
            if (!imageDatName) {
